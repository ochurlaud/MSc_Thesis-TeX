...
\subsubsection{Best correction at BESSY~II}
Following the design rules defined in \cite{book:Skogestad-2005}, one can synthesize an $\mathcal{H}_\infty$ controller, which is an optimal controller obtained by optimizing the closed and opened loop over the $\infty$-norm with given parameters. The Robust Control Toolbox from \textsc{Matlab} was used for this effect.

The 
\todo[inline]{Optimal correction: Maybe theory in first part and only results at the end?}

\section{Summary}
\todo[inline]{CTL : Conclusion}