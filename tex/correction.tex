...
\todo{description of IOC, RFM etc}
...
One difficulty in improving the current correction process is that machine time is expensive. A wrong correction can destabilize the orbit or worse, lose the electrons in the vacuum chamber walls, and thus it cannot be tested when the storage ring is on service or other scientists do experiments or measurements for their own research. Simulating the whole environment is expected to mitigate this.
...
\begin{sidewaysfigure}
    ...
    % We start by placing the blocks
    \node [input] (xref) {};
    \node [block, right=of xref] (ADC1) {$\downarrow$ \SI{150}{\hertz}};
    \node [operator, right=of ADC1] (sum) {$+$};
    \node [block, right=of sum] (correct) {Correction};
    \node [block, right=of correct] (DAC) {$\uparrow$};
    \node [block, right=of DAC] (delay) {Delay T};
    \node [block, right=of delay] (Hdip) {$\text{H}_\text{dipole}$};
    \node [operator, right=of Hdip] (sumperturb) {$+$};
    ...
    % Once the nodes are placed, connecting them is easy. 
    \draw [->] (xref)     -- node {$\vec{X}_\text{ref}$} (ADC1);
    \draw [->] (ADC1)     -- node [pos=0.85, below]{$-$}    (sum);
    \draw [->] (sum)      -- node {$\Delta\vec{X}$}    (correct);
    \draw [->] (correct)  -- node {}                   (DAC);
    \draw [->] (DAC)      -- node {}                   (delay);
    \draw [->] (delay)    -- node[above] {}            (Hdip);
    \draw [->] (Hdip)     -- node {}                   (sumperturb);
