...
\todo{explain somewhere that that it's continuous in the kick so for a sine the solution must be....}
...
The kick is then between $\phi_{k_0-1}$ and $\phi_{k_0+1}$\todo{The code doesn't do that for now}, and we assess that the closest sine is $\tilde{x}(\phi) = b \sin(\phi + c) $.
...
\subsubsection{Case of a unique perturbation source}
