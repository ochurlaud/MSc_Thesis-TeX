...
\tikzset{
    ...
    operator/.style= {draw, fill=white, circle, node distance=2cm},
...
\begin{figure}
    ...
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \node [input] (xref) {};
        \node [input] (input) {};
        \node [operator, below of=xref] (sum) {$+$};
        \node [operator, right of=sum] (prod) {$\times$};
        \node [input, above of=prod] (invS) {};
        \node [gain, right of=prod] (gain) {$\vec{W}$};
        \node [block, right of=gain] (PID) {PID};
        \node [input, above of=PID] (pidcoef) {};
        \node [operator, right of=PID] (sum2) {$+$};
        \coordinate [right of=sum2, right=0.8cm] (straight) {};
        \node [block, above of=straight, left=0.8cm] (delay) {T};
        \node [draw, ellipse, minimum height=1.5cm, minimum width=6cm, below of=straight] (ring) {Storage Ring};
        \coordinate [below of = sum, left=1cm] (leftback) {};
        
        % Once the nodes are placed, connecting them is easy. 
        \draw [->] (xref) -- node [pos=0.85, left]{$-$} node {$\vec{X}_\text{ref}$}(sum);
        \draw [->] (sum) -- node {$\Delta\vec{X}$} (prod);
        \draw [->] (invS) -- node {$S^*$} (prod);
        \draw [->] (prod) -- node {} (gain);              
        \draw [->] (gain) -- node {} (PID);
        \draw [->] (PID) -- node {} (sum2);
        \draw [->] (pidcoef) -- node[pos=0.70, above, fill=white] {PID coeff.} (PID);
        \draw [-] (sum2) -- node {} (straight);
        \draw [->] (straight) |- node {} (delay);
        \draw [->] (delay) -| node[pos=0.92, left] {$-$} (sum2);
        \draw [->](straight) -- node {} (ring.north);
        \draw [-] (ring) -- node{}  (leftback);
        \draw [->] (leftback) |- node {$\vec{X} $}  (sum);
\end{tikzpicture}
